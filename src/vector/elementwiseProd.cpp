...
double *vectorsElementwiseProdPreDetermined(double *a, double *b, int n, MPI_Comm comm)
{
    int rank, commSize = 0;
...
double *vectorElementwiseProdSequential(double *a, double *b, int m, MPI_Comm comm)
{
    int rank, commSize = 0;
...
double *vectorElementwiseProduct(double *a, double *b, int m, MPI_Comm comm)
{
    ...
    if (m < 100)
    {
        return vectorElementwiseProdSequential(a, b, m, comm);
    }
    else
    {
        return vectorsElementwiseProdPreDetermined(a, b, m, comm);
    }
}