...
double *subScalarToVectorSequential(double *a, double x, int m, MPI_Comm comm)
{
    int rank, commSize = 0;
...
double *subScalarVector(double *a, double x, int m, MPI_Comm comm)
{
    ...
    if (m < 100)
    {
        return subScalarToVectorSequential(a, x, m, comm);
    }
    else
    {
        return vectorScalarSubPreDetermined(a, x, m, comm);
    }
}